...
function getData(ParsedData){
  var col = [];
                for (var i = 0; i < ParsedData.length; i++) {
                    var rec = {};
                    rec.id = ParsedData[i].id;
                    rec.name = ParsedData[i].name;
                    rec.username = ParsedData[i].username;
                    rec.city = ParsedData[i].address.city;
                    rec.zipcode = ParsedData[i].address.zipcode;
                    rec.phone = ParsedData[i].phone;
                    rec.bs = ParsedData[i].company.bs;
                    col.push(rec);
                }
...
function Submitvalidate(event) {
    if ((validateName(event) || validateUserName(event) || validateCity(event) || validateZipcode(event) || PhoneValidation(event) || BsValidation()) != true) {
        event.preventDefault();
    } else if ((validateName(event) && validateUserName(event) && validateCity(event) && validateZipcode(event) && PhoneValidation(event) && BsValidation()) == true) {
        newDataObject(event);
    }
}